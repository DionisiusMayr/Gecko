...
def get_collection_of_user(user: str, queued_users: List[str]) -> str:
    ...
    fkey = f'boardgame/collection/{user}.xml'

    r_text = S3.get_file(BUCKET, fkey)
    if r_text:
        return r_text
    else:
    ...
    # else:
        ...
        if r.status_code == 200:
            ...
            #     f.write(r.text)
            fkey = f'boardgame/collection/{user}.xml'
            S3.put_in_bucket(BUCKET, fkey, r.text)
...
def request_users_who_rated_game(game_id: int, page: int, queued_games: List[str]) -> str:
    ...
    if r.status_code == 200:
        ...
        # with open(fname, 'w') as f:
            # f.write(r.text)
            # return r.text
        fkey = f'boardgame/boardgame/{game_id}'
        S3.put_in_bucket(BUCKET, fkey, r.text)
    elif r.status_code == 202:
        # Request is in the queue
...
def grab_all_pages(game_id: int, root, queued_games):
    ...
    for pnumber in range(2, total_pages):
        ...
        fkey = f'boardgame/boardgame/{game_id}/{pnumber}.xml'
        r_text = S3.get_file(BUCKET, fkey)
        if not r_text:
        # if os.path.isfile(fname):
        #     # No need to call the API again
...
def get_reviews_of_game(game_id: int, queued_games: List[str]) -> List[str]:
    ...
    fkey = f'boardgame/boardgame/{game_id}/{page}.xml'
    print(BUCKET, fkey)
    r_text = S3.get_file(BUCKET, fkey)
    if not r_text:
    # if os.path.isfile(fname):
...
def users_who_rated_game(game_id: int) -> List[str]:
    ...
    xmls = S3.get_keys_with_prefix(BUCKET, f'boardgame/boardgame/{game_id}')

    for xml in xmls:
        # Each xml here corresponds to a page of reviews
        # fname = f'{path}/{xml}'

        fkey = f"boardgame/boardgame/{game_id}/{xml.split('/')[-1]}"
        r_text = S3.get_file(BUCKET, fkey)
        # with open(fname, 'r') as f:
            # r_text = f.read()
...
if __name__ == "__main__":
    ...
    # A Feast for Odin
    get_reviews_of_game(game_id=177736, queued_games=queued_games)
    users = users_who_rated_game(game_id=177736)
